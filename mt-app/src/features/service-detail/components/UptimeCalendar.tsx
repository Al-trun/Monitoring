...
export function UptimeCalendar({ serviceId, refreshKey }: UptimeCalendarProps) {
  ...
  if (loading) {
    return (
      <div className="p-6 rounded-xl border border-slate-200 dark:border-[#283039] bg-white dark:bg-[#1a2129] animate-pulse">
        ...
        <div className="grid grid-cols-[repeat(30,1fr)] gap-1 mb-4">
          {Array.from({ length: 90 }).map((_, i) => (
            <div key={i} className="h-3 rounded-[1px] bg-slate-200 dark:bg-slate-700" />
          ))}
        </div>
  ...
  return (
    <div className="p-6 rounded-xl border border-slate-200 dark:border-[#283039] bg-white dark:bg-[#1a2129]">
      {/* Header */}
      ...
      {/* Date Range */}
      <div className="flex justify-between text-slate-400 dark:text-[#6b7c8d] text-[10px] font-bold uppercase tracking-wider mb-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between text-sm">
          <span className="text-slate-500 dark:text-[#9dabb9]">{t('services.detail.uptime')}</span>
