...
import { AddChannelModal } from '../features/alerts/components/AddChannelModal';
  ...
  return (
    <>
      ...
      >
        {activeTab === 'channels' && (
          <button
            onClick={() => { setEditingChannel(undefined); setIsModalOpen(true); }}
        ...
        {tabs.map(tab => (
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === tab.key
              ...
              : 'text-slate-500 dark:text-text-muted-dark hover:text-slate-700 dark:hover:text-slate-200'
              }`}
          >
          ...
          <div className="bg-white dark:bg-bg-surface-dark border border-slate-200 dark:border-ui-border-dark rounded-xl overflow-hidden">
            {isLoading ? (
              ...
              <div className="p-12 text-center">
                <MaterialIcon name="notifications_off" className="text-6xl text-slate-300 dark:text-slate-700 mb-4" />
              ...
              <div className="divide-y divide-slate-200 dark:divide-slate-800">
                {channels.map((channel) => (
                  <div
                    key={channel.id}
                    className={`p-4 flex items-center gap-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors ${!channel.isEnabled ? 'opacity-50' : ''}`}
                  >
                    <div
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {!channel.isEnabled && (
                          <span className="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400 rounded-full">
